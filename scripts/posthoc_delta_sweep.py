    ...
    for p, g in zip(all_pred_sets, all_gold_sets):
        ...
        for label in LABELS:
            ...
            if label in p and label in g:
                tp[label_idx] += 1
            elif label in p and label not in g:
                fp[label_idx] += 1
            elif label not in p and label in g:
                fn[label_idx] += 1
    
    # 计算micro和macro F1
    micro_f1 = f1_from_counts(sum(tp), sum(fp), sum(fn))
    
    macro_f1_sum = 0.0
    for i in range(L):
        macro_f1_sum += f1_from_counts(tp[i], fp[i], fn[i])
    macro_f1 = macro_f1_sum / L
